...
let updateSun = (sunElement, minutesSunUp, totalMinutes) => {
  ...
  console.log('y ' + y);
  var x = percentage > 100 || percentage < 0 ? document.querySelector('.is-day').classList.add('is-night') : document.querySelector('.is-day').classList.remove('is-night');
...
const updateTimeAndTimeLeft = (sunset) => {
  ...
  if (hours > 0 && minutes >= 0) {
    document.querySelector('.js-time-left').innerText = `${hours} hours ${minutes} minutes`;
    ...
    document.querySelector('.js-summary').innerHTML = `<span class="js-time-left">4 minutes less</span><span class="u-muted"> sunlight today. unfortunately. It&#8217ll get better!</span>`;
    if (hours2 > 0 && minutes2 >= 0) {
      document.querySelector('.js-time-left').innerText = `${hours2} hours ${minutes2} minutes less`;
    } else if (hours2 <= 0 && minutes2 > 0) {
      document.querySelector('.js-time-left').innerText = `${minutes2} minutes less`;
    }
  }
};
